...
class WeatherDatabaseMigrationTest {
    ...
    @Test
    fun migrate8To9_addsCountryCodeColumn() {
        helper.createDatabase(TEST_DB, 8).apply {
            execSQL(
                ...
                CREATE TABLE IF NOT EXISTS location_entity (
                    ...
                    lastViewedAt INTEGER NOT NULL DEFAULT 0,
                    imageUrl TEXT,
                    placeMetadataUpdatedAt INTEGER NOT NULL DEFAULT 0
                )
                """.trimIndent()
            )
            execSQL(
            ...
            execSQL(
                ...
                INSERT INTO location_entity (
                    id, name, latitude, longitude, country, admin1, lastUpdated,
                    hasSeaAccess, lastViewedAt, imageUrl, placeMetadataUpdatedAt
                ) VALUES (1, 'Lisbon', 38.7, -9.1, 'Portugal', 'Lisbon', 1, 1, 99, 'https://example.com/x.jpg', 0)
                """.trimIndent()
            )
            close()
        }
        ...
        db.query(
            "SELECT countryCode, imageUrl FROM location_entity WHERE id = 1"
        ).use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertTrue(cursor.isNull(cursor.getColumnIndexOrThrow("countryCode")))
            assertEquals("https://example.com/x.jpg", cursor.getString(cursor.getColumnIndexOrThrow("imageUrl")))
        }
        db.close()
    }
