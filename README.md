...
- **7-Day Forecast (core)**: the detail sheet is **locked at 60% of the screen** (map layout height ~40%) — it does not drag to full screen or collapse. **One outer `verticalScroll`** covers the whole detail body (same pattern as home): **16:9 city photo** flush to the sheet top and horizontal edges (header overlaid on a top scrim), then a **compact equal-width row of 7 day chips** (weekday + date + weather icon, high/low, precip — no activity on the chip), then a **column of ranked activities** for the selected day. Today is first and selected (`selectedDayIndex = 0`). Tapping a day only swaps the activity list. There is no "7-day forecast" heading and no "Wednesday's picks" title.
- **Per-day recommendations**: ranked activities as a **compact list** (icon + name + score + a localized **why** line from `ReasonKey`) — informational rows, not tappable cards. The sheet scrolls so all rows fit. There is no single "week-long" score.
...
2. Home peek shows ~60% **map layout height**. Drag the sheet up: the map shrinks with the sheet. Open a city: detail **locks at 60%** (map stays ~40% tall); **16:9 overlay hero** flush to the sheet top (header on scrim); **7 compact day chips**; activities below — **outer sheet scroll** (`detail_sheet_body`) if they overflow. The detail sheet itself does not expand.
...
**Run manually**: `./gradlew installDebug` → search in header → detail (sheet locked at 60%) → scroll the sheet (hero, days, then activities) → Wikipedia mark opens the article → tap day chips → toggle dark mode (map tiles follow) → share. Back to home: peek ~40% sheet / 60% map, drag to full screen and watch the map shrink.
...
| Advanced UI polish / animation | Done | Home peek ~40% (map layout ~60%), expandable (map shrinks with sheet offset); detail locked at 60% with one outer scroll; 16:9 overlay city-image hero; cache-miss 350+1200 ms fly (sheet −500 ms); cached 150+600 ms (sheet −200 ms); detail shimmer matches overlay hero + compact day chips + activity rows |
| Dark mode + theme toggle | Done | Material 3 dark surface tokens (`#1C1B1F` + tonal containers); sheet-header theme toggle; DataStore preference; **first launch** Dark/Light from sunrise/sunset at GPS (clock 19:00–06:00 fallback); OpenFreeMap Liberty vs Dark tiles follow immediately |
