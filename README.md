...
| **Stretch** | Home top picks (postcard cards + Top Picks / Recent tabs); History (10 cities, Room `lastViewedAt`, id dedupe); Marine API; Wikipedia city thumbnails (Room v8 `imageUrl` + 30-day `placeMetadataUpdatedAt`; v9 `countryCode`; MediaWiki redirects + Coil prefetch); WorkManager sync; pull-to-refresh; dark mode + **Light / Dark / Cycle** theme toggle (Cycle is default: solar night at GPS, else clock 19:00–06:00); splash + original mark; share 9:16 weather flyer PNG (+ city-image background, save to Downloads); MapLibre map **sized with the sheet** (home peek ~40% / map ~60%, expandable; detail **locked at 60%** / map ~40%); GPS current-location chip; nearby-city prefetch; country-warm catalog (capitals then majors); Paparazzi + instrumented CI |

Key experience details:
- **7-Day Forecast (core)**: the detail sheet is **locked at 60% of the screen** (map layout height ~40%) — it does not drag to full screen or collapse. **16:9 city photo** flush to the sheet top (header overlaid on a top scrim); on a short device the hero shrinks so chips + rows get a minimum height. Then **tall equal-width day chips** (weekday + date + weather icon, high/low, precip — no activity on the chip), then **compact ranked rows** (icon → score → title + reason). When leftover height cannot fit those rows, the chips+list column **scrolls** (`detail_sheet_scroll`). On taller phones the rows still share leftover height via `Modifier.weight`. Today is first and selected (`selectedDayIndex = 0`). Tapping a day only swaps the activity list. There is no "7-day forecast" heading and no "Wednesday's picks" title.
...
- **Recently viewed History**: the **Recent** tab lists up to 10 cities the user explicitly opened (search, top-pick, or map tap). Persisted via Room `lastViewedAt`. Map taps reverse-geocode via Nominatim then **stabilize** to a nearby Open-Meteo / GeoNames id (or a cached Room row) so search and tap share a cache key; remaining collisions collapse by proximity/name.
...
- **First-run theme**: default stored mode is **Cycle** (not a sticky Light/Dark). Cycle keeps following day/night until the user locks Light or Dark: solar elevation at the device lat/lng when GPS is available, otherwise local clock 19:00–06:00. The header control cycles **Light → Dark → Cycle**. Light/Dark are sticky user locks; Cycle is an explicit stored mode. Legacy "system" rows map to Cycle.
- **Cache windows**: daily Open-Meteo forecasts skip the network when Room is newer than **6 hours** (global model cadence; matches WorkManager). City Wikipedia thumbnails reuse a stored URL for **30 days** when `placeMetadataUpdatedAt` is set (`0` from Room v8 migration is stale); a confirmed miss retries after **30 minutes**. Lookup follows MediaWiki redirects (`La Habana` → `Havana`), then `"name, country"`, then `list=search`, and Coil prefetches bytes after Room stores a URL. Weather persists before Wikipedia so day chips paint first; `updatePlaceImage` fills the hero via the Room flow. After a city loads, nearby hubs (`HubCities`) in the same map region are prefetched into Room so the next tap can paint from cache. When GPS resolves a country (ISO or country-name fallback, including UK→GB), the app progressively warms that country’s bundled catalog (admin capitals by population, then other majors) — **+8** after GPS, **+2** after each selection (still the **device** country while GPS is present), 2 s apart. The **budget is session RAM** (a cold start re-issues +8 on the next GPS fix); warmed forecasts persist in Room. Only weather stays on the 6 h refresh path.
...
2. Home peek shows ~60% **map layout height**. Drag the sheet up: the map **resizes** with sheet offset. Open a city: detail **locks at 60%** (map stays ~40% tall, swipe disabled); **16:9 overlay hero** flush to the sheet top (header on scrim); **tall day chips**; compact icon→score→title+reason rows — they fill leftover height on a tall phone and **scroll** if the sheet is too short. The detail sheet itself does not expand.
3. Tap a day chip → activities re-rank. Wikipedia **W** opens the city article in the browser (`ACTION_VIEW`, no dialog). Toggle theme (Light / Dark / Cycle): map tiles should switch Liberty ↔ Dark immediately.
4. Open a second nearby city (map tap or search) — if it was prefetched, detail should appear as the camera lands.
5. Search **La Habana** (or grant location in Cuba): the 16:9 hero should fill via Wikipedia redirects, not stay grey for 6 hours. A miss shows shimmer then a tonal fallback and retries after 30 minutes.
6. Grant location: after the current-location chip appears, country-warm runs in the background for the **device** country (ISO or country name; browsing Madrid from Lisbon still warms Portugal). Open another city in that country later — it should often paint from Room.
7. Share flyer still exports a 9:16 poster (city image when cached).
8. Fresh install: Cycle is stored (appearance follows day/night live). Locking Light or Dark is sticky; Cycle keeps tracking dusk.
...
| Cache windows | `CachePolicy` (6 h weather / 30 d image hit / 30 min image miss) + `CountryCityCatalog` GPS warm |
| Wikipedia place image | `WikipediaPlaceImageResolver` (redirects → `"name, country"` → search; thumbnail page preferred) + Coil `PlaceImagePrefetcher` |
...
| Nominatim in-flight cap | `HostConcurrencyLimiter` (max 1 to `*.openstreetmap.org`) |
...
- **Clean Architecture Principles**: The project is strictly divided into three layers:
  - **Data Layer**: Retrofit interfaces (Open-Meteo Forecast / Geocoding / Marine, Nominatim, Wikipedia), Room DAOs, bundled `country_cities.json` via `CountryCityCatalogLoader`, and the Repository implementation mapping network data to domain models.
...
- **Offline-First (SSOT)**: The UI never consumes data directly from the network. It observes a Room database `Flow`. A parallel background request fetches fresh data from the Open-Meteo API and updates the local database, triggering reactive UI updates. Selecting a city starts fetch immediately, plants the pin, then `hasFreshForecast` picks a `MapHopProfile` (snappy if Room weather is within 6 h). The sheet waits `hop.contentRevealMs` so the map still leads. Room collect and `refreshForecast` are sibling coroutines under one `forecastJob` so a later selection cancels both. Weather refresh is skipped when `lastUpdated` is within `CachePolicy.WEATHER_TTL_MS` (6 h), but a missing place image still retries via `shouldFetchPlaceImage` (30 d hit / 30 min miss). Forecast+marine run in parallel; Wikipedia patches `imageUrl` afterward. Persist merges an existing Room `countryCode` so nearby/search refreshes do not wipe the ISO; a TTL skip still **backfills** a missing ISO. After a successful city load, `prefetchNearbyCities` warms `HubCities` neighbors in the same ~280 km region. When GPS (ISO or country-name fallback) or a no-GPS selection with an ISO resolves a country, `prefetchCountryCities` progressively warms that catalog (never-viewed rows evict first; viewed overflow is `lastViewedAt` LRU; cap **100**).
- **Per-lane fetch state**: Search, Top Picks, forecast, and map-tap are independent async lanes (`SearchUiState` / `FetchStatus`, including `Failed`). A search error does not reuse the forecast banner. Pull-to-refresh can show existing Top Picks while `FetchStatus.Refreshing`.
...
- **Data Layer**: `WeatherRepositoryImpl` tested with MockK for APIs and DAO (including weather TTL skip, Wikipedia 30-day reuse / 30-minute miss, country-warm skip/limit, `countryCode` merge, ISO backfill on TTL skip, Nominatim→GeoNames `rekeyLocation`, `UnknownHostException` vs generic `IOException`, and `getForecastFlow` following a rekey); `LocationSyncer` (viewed + 16 unviewed cap), `HubCities` / `NearbyCities`, `CountryCityCatalog`, `WikipediaPlaceImageResolver` (thumbnail page preferred), `HostConcurrencyLimiter` (Open-Meteo 3 / Nominatim 1), and rate-limit retry interceptor covered.
...
|-------|--------|----------------|
| Offline cache | Done | Room SSOT + 6 h forecast TTL (`CachePolicy`) + WorkManager 6 h **weather-only** upkeep (viewed + 16 unviewed); 30 d image hit / 30 min miss; nearby-hub prefetch; country-warm catalog (19 ISO codes) |
...
| Advanced UI polish / animation | Done | Home peek ~40% (map layout ~60%), expandable (map **resizes** with `requireOffset()`); detail locked at 60% / map ~40%; chips+rows **scroll** when the leftover height is too short; 16:9 overlay hero (shrinks on short sheets); cache-miss 350+1200 ms fly (sheet −500 ms); cached 150+600 ms (sheet −200 ms); detail shimmer matches overlay hero + tall day chips + activity rows |
...
- **Geocoding API**: `https://geocoding-api.open-meteo.com/v1/search` for resolving a city name to coordinates. We also read `elevation`, `population`, `feature_code`, and `country_code` (ISO alpha-2) from each result to drive geography-aware activities, home suggestions, and country-warm when GPS is absent.
...
- **Reverse geocode** (map tap / long-press): Open-Meteo has no reverse endpoint, so we call [Nominatim](https://nominatim.openstreetmap.org/) (`/reverse`) with a descriptive `User-Agent`, then stabilize to a nearby Open-Meteo / GeoNames id when one exists (keeping Nominatim `country_code` for country-warm), and open the same detail flow as `onLocationSelected`.
...
- **Home "top picks"** come from a curated, bundled `FeaturedCities` list (`HubCities.featured`) because the Geocoding API only supports search-by-name (no discovery/browse endpoint). Selection is randomised but weighted by population. Seed location IDs are **synthetic negatives** shared with map hubs (`HubCities`, mostly −101… plus a few featured-only ids) — they are **not** Open-Meteo / GeoNames IDs — so they cannot collide with real place IDs returned by search. Map-neighborhood prefetch uses the same `HubCities.all` list. Country-warm catalog rows use **−200_000 − index** so they never collide with hubs or Nominatim (−1_000_000…). The bundled `country_cities.json` covers **19 ISO codes / 236 cities** (not a global gazetteer).
...
- **WorkManager `LocationSyncer`**: keeps Room forecasts fresh for previously viewed cities (and a bounded set of country-warm prefetch rows) without blocking the UI.
...
- **Rate limiting (HTTP)**: GET requests retry on HTTP 429 with exponential backoff (respecting `Retry-After` when present). Concurrent Open-Meteo calls (forecast + geocoding + marine) are capped at **3 in-flight** by `HostConcurrencyLimiter`; Nominatim is capped at **1 in-flight** (`*.openstreetmap.org`); Wikipedia is not gated. Timeouts (`SocketTimeoutException`, HTTP 408) map to `AppError.NetworkError.Timeout`; `UnknownHostException` / `ConnectException` / `NoRouteToHostException` map to `NoConnectivity`; other `IOException`s (limiter wait, broken streams) map to `NetworkError.Unknown`; TLS failures are not treated as offline.
