...
| **Stretch** | Home top picks (postcard cards + Top Picks / Recent tabs); History (10 cities, Room `lastViewedAt`, id dedupe); Marine API; Wikipedia city thumbnails (Room v8 `imageUrl` + 30-day `placeMetadataUpdatedAt`); WorkManager sync; pull-to-refresh; dark mode + **persisted theme toggle** (first launch follows sunrise/sunset at the device location); splash + original mark; share 9:16 weather flyer PNG (+ city-image background, save to Downloads); MapLibre map under a Material 3 bottom sheet (home peek ~40% / map ~60%, expandable; detail **locked at 60%**); GPS current-location chip; nearby-city prefetch; Paparazzi + instrumented CI |

Key experience details:
- **7-Day Forecast (core)**: the detail sheet is **locked at 60% of the screen** (map ~40% visible) — it does not drag to full screen or collapse. Overflow **scrolls inside** the sheet. Top of the sheet is an **overlay hero**: city photo edge-to-edge under a scrim, with back / city / Wikipedia / share / theme on the image. Below: **seven tall day-of-week buttons in one `fillMaxWidth` row** (weekday + date stacked, weather icon, high/low, precip — no activity name/score on the chip). Today is first and selected (`selectedDayIndex = 0`). Tapping a day re-ranks the **vertical activity list** below. There is no "7-day forecast" heading and no "Wednesday's picks" title.
...
2. Home peek shows ~60% map. Drag the sheet up to full screen; home list scrolls. Open a city: detail **locks at 60%** (map stays ~40% visible); overlay hero flush to the sheet top (header on scrim); **7 tall day buttons**; activities below — **inner-scroll** if they overflow. The sheet itself does not expand.
3. Tap a day button → activities re-rank. Wikipedia mark opens the city article in the browser. Toggle theme: map tiles should switch Liberty ↔ Dark immediately.
...
**Run manually**: `./gradlew installDebug` → search in header → detail (sheet locked at 60%) → inner-scroll activities → Wikipedia mark opens the article → tap day buttons → toggle dark mode (map tiles follow) → share. Back to home: peek ~40%, drag to full screen.
...
| Advanced UI polish / animation | Done | Home peek ~40% (map ~60%), expandable; detail locked at 60% with inner scroll; overlay city-image hero; cache-miss 350+1200 ms fly (sheet −500 ms); cached 150+600 ms (sheet −200 ms); detail shimmer matches overlay hero + 7 tall day buttons + activity rows |
...
| In-screen map | Done | Full-screen MapLibre under a Material 3 bottom sheet (home peek 40% / map 60%, expandable; detail locked 60%); OpenFreeMap Liberty (light) / Dark (dark) tiles; no overlay AppBar; map-first hop; tap → Nominatim reverse; camera/pin in ViewModel |
...
The map is a **full-screen background** under a Material 3 `BottomSheetScaffold`. **Home** peek height is **40% of the screen** (about **60% of the map** stays visible); dragging expands the sheet to full screen and home content **scrolls**. **Detail** peek is **60%** and the sheet is **locked** (`sheetSwipeEnabled = false`); only the detail column scrolls. Detail clips a borderless city photo to the sheet’s rounded top corners with the header on a top-down scrim. `mapCamera` / `mapPin` live in `WeatherUiState` and drive the same map instance on select/back. Home centers on the device location when available (static London default otherwise); back returns to that same overview.
