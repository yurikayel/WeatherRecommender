...
- **Cache windows**: daily Open-Meteo forecasts skip the network when Room is newer than **6 hours** (global model cadence; matches WorkManager). City Wikipedia thumbnails reuse a stored URL for **30 days** when `placeMetadataUpdatedAt` is set (`0` from Room v8 migration is stale); a confirmed miss retries after **30 minutes**. Lookup follows MediaWiki redirects (`La Habana` → `Havana`), then `"name, country"`, then `list=search`, and Coil prefetches bytes after Room stores a URL. Weather persists before Wikipedia so day chips paint first; `updatePlaceImage` fills the hero via the Room flow. After a city loads, nearby hubs (`MajorCities` ∪ `FeaturedCities`) in the same map region are prefetched into Room so the next tap can paint from cache. When GPS resolves a country (ISO or country-name fallback, including UK→GB), the app progressively warms that country’s bundled catalog (admin capitals by population, then other majors) — **+8** after GPS, **+2** after each selection (still the **device** country while GPS is present), 2 s apart. The **budget is session RAM** (a cold start re-issues +8 on the next GPS fix); warmed forecasts persist in Room. Only weather stays on the 6 h refresh path.
