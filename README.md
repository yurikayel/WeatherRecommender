...
- **7-Day Forecast (core)**: the detail sheet is **locked at 60% of the screen** (map layout height ~40%) — it does not drag to full screen or collapse. The detail body **does not scroll**: **16:9 city photo** flush to the sheet top and horizontal edges (header overlaid on a top scrim), then a **compact equal-width row of 7 day chips** (weekday + date + weather icon, high/low, precip — no activity on the chip), then a **weighted column of ranked activities** that always shows every applicable row in the leftover space. Today is first and selected (`selectedDayIndex = 0`). Tapping a day only swaps the activity list. There is no "7-day forecast" heading and no "Wednesday's picks" title.
...
| Advanced UI polish / animation | Done | Home peek ~40% (map layout ~60%), expandable (map shrinks with sheet offset); detail locked at 60% with a non-scrolling weighted activity remainder; 16:9 overlay city-image hero; cache-miss 350+1200 ms fly (sheet −500 ms); cached 150+600 ms (sheet −200 ms); detail shimmer matches overlay hero + compact day chips + activity rows |
