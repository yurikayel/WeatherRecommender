...
- **Rate limiting (HTTP)**: GET requests retry on HTTP 429 with exponential backoff (respecting `Retry-After` when present). Concurrent Open-Meteo calls (forecast + geocoding + marine) are capped at **3 in-flight** by `HostConcurrencyLimiter`; Nominatim is capped at **1 in-flight** (`*.openstreetmap.org`); Wikipedia is not gated. Timeouts (`SocketTimeoutException`, HTTP 408) map to `AppError.NetworkError.Timeout`; generic `IOException` maps to `NoConnectivity`; TLS failures are not treated as offline.
